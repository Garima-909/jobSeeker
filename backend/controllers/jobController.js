...
export const postJob = catchAsyncError(async(req, res, next) => {
    ...
    if((!salaryFrom || !salaryTo) && !fixedSalary){
        return next(new ErrorHandler("Please provide either fixed or range of salary!", 400));
    }

    if(salaryFrom && salaryTo && fixedSalary){
        return next(new ErrorHandler("Fixed salary and ranged salary cannot be entered together!", 400));
    }
...
export const updateJob = catchAsyncError(async(req, res, next) => {
    ...
    job = Job.findByIdAndUpdate(id, req.body, {
        new: true,
        runValidators: true,
